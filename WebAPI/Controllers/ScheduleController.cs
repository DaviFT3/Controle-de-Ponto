...
namespace WebAPI.Controllers
{
    ...
    public class ScheduleController : ControllerBase
    {
        ...
        public IActionResult GetSchedulesByCollaboratorId([FromQuery] int id, [FromQuery] int year, [FromQuery] int month)
        {
            ...
            return Execute(() => _Scheduleservice.GetAllByCollaboratorIdAndYearAndMonth(id, year, month));
        }
        [HttpGet]
        ...
        [HttpGet]
        [Route("DashboardDates")]
        public IActionResult DashboardDates(int id)
        {
            if (id == 0)
                return NotFound();

            return Execute(() => _Scheduleservice.GetLastRegisters(id));
        }
        [HttpGet]
        [Route("HoursBalance")]
        public IActionResult Balance(int id)
        {
            if (id == 0)
                return NotFound();
            return Execute(() => _Scheduleservice.HoursBalance(id));
        }
